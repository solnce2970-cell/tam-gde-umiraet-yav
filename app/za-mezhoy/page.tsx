  ...
  return (
    <main className="beyondPage">
      ...
      <section className="hero">
        ...
        <p className="heroIntro">Это место существует только для тех, кого сайт уже запомнил.</p>
      </section>

      <section className="rewardBanner" aria-label="Награда за знаки Межи">
        <p>Когда будут найдены все 13 знаков Межи, здесь откроются тайные сказания — отрывки, которых нет в тексте романа.</p>
      </section>
...
const styles = `
  *{box-sizing:border-box}
  ...
  .pageFooter{text-align:center;padding:0 5vw 70px;color:#777168;font:12px Arial,sans-serif;letter-spacing:.05em}.pageFooter b{color:#b9935a;font-weight:400}.pageFooter a{display:inline-block;margin-top:14px;color:#bba67f;text-decoration:none;text-transform:uppercase;font-size:10px;letter-spacing:.14em}
  @media(max-width:900px){.memoryGrid{grid-template-columns:repeat(2,minmax(0,1fr))}}
  @media(max-width:620px){.topbar{padding:16px 18px}.topbar a:first-child{font-size:11px}.hero{min-height:calc(100svh - 52px);padding:48px 16px 54px;background-image:linear-gradient(180deg,rgba(5,8,6,.22) 0%,rgba(5,8,6,.44) 42%,rgba(7,10,8,.9) 100%),radial-gradient(circle at 50% 34%,rgba(5,8,6,.04) 0%,rgba(5,8,6,.3) 72%),url('/assets/v1/images/za-mezhoy/za-mezhoy-mobile.webp');background-position:center top}.hero h1{margin-bottom:28px;font-size:clamp(54px,18vw,76px);line-height:.94}.heroIntro{max-width:330px;margin-top:25px;font-size:14px;line-height:1.65}.progressSeal{width:178px;height:178px}.sealHalo{inset:17px}.sealMarks span{font-size:11px}.sealCore{width:104px;height:104px}.sealCore strong{font-size:44px}.rewardBanner{padding:24px 14px 0}.rewardBanner p{padding:20px 14px;font-size:17px;line-height:1.6}.memoryGrid{grid-template-columns:1fr;padding:40px 14px 70px}.memoryCard{min-height:205px}.finalSecret{min-height:200px;margin:0 14px 64px}}
`;
